...
def train(X, actor, critic, decodings, out_dir=None):
    ...
    for e in range(EPOCHS):
        ...
        np.save("History_repeat/in-{}.npy".format(e), org_mols)
        np.save("History_repeat/out-{}.npy".format(e), batch_mol)
        np.save("History_repeat/score-{}.npy".format(e), np.asarray(frs))
        ...
        print("Epoch {2} \t Mean score: {0:.3}\t\t Percentage in range: {1},  {3}".format(
            ...
            round(np.mean(np.sum(frs, 1) == 5), 2)
        ))
