# -*- coding: utf-8 -*-
# @Time    : 2023/2/27 16:30
# @Author  : hxt
# @FileName: amid_graph_encoding.py
# @Software: PyCharm
import torch
...
from karateclub import Graph2Vec, graph_embedding
...
def create_pytorch_geometric_graph_data_list_from_smiles(x_smiles):
    ...
    graph_2_vec=model.get_embedding()
    print(graph_2_vec)
    print(type(graph_2_vec))
    return graph_2_vec
def read_smile_from_file(filename):
    csv_file = pd.read_csv(filename)
    smile_col=(csv_file.iloc[:,1])
    smile_list=[]
    for smile in smile_col:
...
if __name__ == '__main__':
    filename = "../Data/smile.CSV"
    smiles=read_smile_from_file(filename)
    graph_2_vec=create_pytorch_geometric_graph_data_list_from_smiles(smiles)
    print(graph_2_vec.shape)




# def amid_graph_encoding(smile_series):
#     from rdkit import Chem
